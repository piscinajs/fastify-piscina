"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
/*
 * This is a TypeScript port of the original Java version, which was written by
 * Gil Tene as described in
 * https://github.com/HdrHistogram/HdrHistogram
 * and released to the public domain, as explained at
 * http://creativecommons.org/publicdomain/zero/1.0/
 */
require("core-js");
var chai_1 = require("chai");
var index_1 = require("./index");
describe("Histogram encoding", function () {
    it("should encode filling a byte buffer", function () {
        // given
        var histogram = new index_1.Int32Histogram(1, Number.MAX_SAFE_INTEGER, 2);
        histogram.recordValue(42);
        var buffer = index_1.ByteBuffer.allocate();
        // when
        var encodedSize = histogram.encodeIntoByteBuffer(buffer);
        // then
        chai_1.expect(encodedSize).to.be.equal(42);
    });
    it("should decode reading a byte buffer", function () {
        // given
        var histogram = new index_1.Int32Histogram(1, Number.MAX_SAFE_INTEGER, 2);
        histogram.recordValue(42);
        histogram.recordValue(7);
        histogram.recordValue(77);
        var buffer = index_1.ByteBuffer.allocate();
        var encodedSize = histogram.encodeIntoByteBuffer(buffer);
        buffer.position = 0;
        // when
        var result = index_1.AbstractHistogram.decodeFromByteBuffer(buffer, index_1.Int32Histogram, 0);
        // then
        chai_1.expect(result.outputPercentileDistribution()).to.be.equal(histogram.outputPercentileDistribution());
    });
    it("should decode and decompress reading a byte buffer", function () {
        // given
        var base64 = require("base64-js");
        var buffer = new index_1.ByteBuffer(base64.toByteArray("HISTFAAAACB42pNpmSzMwMDAxAABMJqRQf4/GNh/gAgEMwEAkp4I6Q=="));
        // when
        var histogram = index_1.AbstractHistogram.decodeFromCompressedByteBuffer(buffer, index_1.Int32Histogram, 0);
        // then
        chai_1.expect(histogram.getMean()).to.be.equal(42);
        chai_1.expect(histogram.getTotalCount()).to.be.equal(1);
    });
    it("should encode and compress an histogram", function () {
        // given
        var histogram = new index_1.Int32Histogram(1, Number.MAX_SAFE_INTEGER, 3);
        histogram.recordValue(32415482);
        // when
        var buffer = index_1.ByteBuffer.allocate();
        histogram.encodeIntoCompressedByteBuffer(buffer);
        // then
        buffer.resetPosition();
        var decodedHistogram = index_1.AbstractHistogram.decodeFromCompressedByteBuffer(buffer, index_1.Int32Histogram, 0);
        chai_1.expect(decodedHistogram.getValueAtPercentile(50)).to.be.equal(histogram.getValueAtPercentile(50));
    });
    it("should decode and decompress reading a base64 string", function () {
        // given
        var base64String = "HISTFAAAAB542pNpmSzMwMDAxAABzFCaEUoz2X+AMIKZAEARAtM=";
        // when
        var histogram = index_1.decodeFromCompressedBase64(base64String);
        // then
        chai_1.expect(histogram.getMean()).to.be.equal(42);
        chai_1.expect(histogram.getTotalCount()).to.be.equal(1);
    });
    it("should decode and decompress a big base64 string", function () {
        // given
        var base64String = "";
        // when
        var histogram = index_1.decodeFromCompressedBase64(base64String);
        // then
        chai_1.expect(histogram.getTotalCount()).to.be.equal(10000);
    });
    it("should encode and compress an histogram to a base64 string", function () {
        // given
        var histogram = new index_1.Int32Histogram(1, 2, 3);
        histogram.recordValue(42);
        // when
        var base64Histogram = index_1.encodeIntoBase64String(histogram);
        // then
        chai_1.expect(base64Histogram).to.be.equal("HISTFAAAAB94nJNpmSzMwMDABMSMQMzMAAGMUJoJxg9mAgA1TQGm");
    });
    it("should throw an error when trying to decompress an histogram using V1 encoding", function () {
        // given
        var base64V1EncodingString = "HISTIgAAAFd42pNpmazIwMAYxgABTBDKT4GBgdnNYMcCBvsPUBkeBkYGZqA8MwMbAzsDC5DFBCTZgJCDQY1BjkGLQZRBlUEPCB8zWDCYMxgDZZkZhgJgHDibAY8JB/A=";
        // when & then
        chai_1.expect(function () { return index_1.decodeFromCompressedBase64(base64V1EncodingString); }).to.throw("Encoding not supported");
    });
});
//# sourceMappingURL=encoding.spec.js.map